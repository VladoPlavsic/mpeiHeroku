...
def create_stored_procedures_insert() -> None:
    ...
    CREATE OR REPLACE FUNCTION private.insert_grade(varchar(100), varchar(100), text, text, int)
        RETURNS TABLE (id int, name_en varchar(100), name_ru varchar(100), background text, object_key text, order_number int)
        AS $$
        DECLARE
        inserted_id int;
        BEGIN
        INSERT INTO private.grade (name_en, name_ru, object_key, background, order_number) VALUES ($1, $2, $3, $4, $5) 
    ...
    CREATE OR REPLACE FUNCTION private.insert_subject(int, varchar(100), varchar(100), text, text, int)
        ...
        inserted_id int;
        BEGIN
        INSERT INTO private.subject (fk, name_en, name_ru, object_key, background, order_number) VALUES ($1, $2, $3, $4, $5, $6) RETURNING private.subject.id INTO inserted_id;
    ...
    CREATE OR REPLACE FUNCTION private.insert_branch(int, varchar(100), varchar(100), text, text, int)
        ...
        inserted_id int;
        BEGIN
        INSERT INTO private.branch (fk, name_en, name_ru, object_key, background, order_number) VALUES ($1, $2, $3, $4, $5, $6) RETURNING private.branch.id INTO inserted_id;
    ...
    CREATE OR REPLACE FUNCTION private.insert_lecture(int, varchar(100), varchar(100), text, text, text, int)
        RETURNS TABLE (id int, fk int, name_en varchar(100), name_ru varchar(100), description text, background text, object_key text, order_number int)
        AS $$
        DECLARE
        inserted_id int;
        BEGIN
        INSERT INTO private.lecture (fk, name_en, name_ru, description, object_key, background, order_number) VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING private.lecture.id INTO inserted_id;
    ...
    CREATE OR REPLACE FUNCTION private.insert_video(int, varchar(100), text, text, text)
        RETURNS TABLE (id int, url text, name_ru varchar(100), description text, object_key text)
        AS $$
        DECLARE
        inserted_id int;
        BEGIN
    ...
    op.execute("""
    CREATE OR REPLACE FUNCTION private.insert_game(int, varchar(100), text, text, text)
        RETURNS TABLE (id int, url text, name_ru varchar(100), description text, object_key text)
        AS $$
        DECLARE
        inserted_id int;
    ...
    CREATE OR REPLACE FUNCTION private.insert_theory(int, varchar(100), text, text)
        RETURNS TABLE (id int, name_ru varchar(100), description text, object_key text)
        AS $$
        DECLARE
        inserted_id int;
        BEGIN
    ...
    CREATE OR REPLACE FUNCTION private.insert_theory_image(int[], int[], text[], text[])
        ...
        RETURN QUERY (SELECT private.theory_image."order", private.theory_image.url, private.theory_image.object_key FROM private.theory_image WHERE private.theory_image.fk = $1[1] ORDER BY private.theory_image.order);
        END $$ LANGUAGE plpgsql;
    """)
    ...
    CREATE OR REPLACE FUNCTION private.insert_practice_image(int[], int[], text[], text[])
        ...
        SELECT unnest($1), unnest($2), unnest($3), unnest($4);
        RETURN QUERY (SELECT private.practice_image."order", private.practice_image.url, private.practice_image.object_key FROM private.practice_image WHERE private.practice_image.fk = $1[1] ORDER BY private.practice_image.order);
        END $$ LANGUAGE plpgsql;
    """)
...
def create_stored_procedures_select() -> None:
    ...
    CREATE OR REPLACE FUNCTION private.select_theory_image(int)
        ...
        AS $$
        BEGIN
        RETURN QUERY (SELECT private.theory_image.url, private.theory_image."order", private.theory_image.object_key FROM private.theory_image WHERE private.theory_image.fk = $1 ORDER BY private.theory_image.order);
        END $$ LANGUAGE plpgsql;
    """)
    # theory audio
    op.execute("""
    ...
    CREATE OR REPLACE FUNCTION private.select_practice_image(int)
        ...
        AS $$
        BEGIN
        RETURN QUERY (SELECT private.practice_image.url, private.practice_image."order", private.practice_image.object_key FROM private.practice_image WHERE private.practice_image.fk = $1 ORDER BY private.practice_image.order);
        END $$ LANGUAGE plpgsql;
    """)
    # practice audio
    op.execute("""
...
def remove_order_number_from_functions() -> None:
    ...
    op.execute("""
    CREATE OR REPLACE FUNCTION private.select_grades_by_ids(text)
        RETURNS TABLE (id int, name_en varchar(100), name_ru varchar(100), background text, object_key text)
        AS $$
        DECLARE 
            ids INT[];
        BEGIN
    ...
    op.execute("""
    CREATE OR REPLACE FUNCTION private.select_subjects_by_ids(text, int)
        RETURNS TABLE (id int, fk int, name_en varchar(100), name_ru varchar(100), background text, object_key text)
        AS $$
        DECLARE 
            ids INT[];
        BEGIN
    ...
    op.execute("""
    CREATE OR REPLACE FUNCTION private.select_all_lectures(int)
        RETURNS TABLE (id int, fk int, name_en varchar(100), name_ru varchar(100), description text, background text, object_key text)
        AS $$
        BEGIN
...
def create_stored_procedures_update() -> None:
    ...
    CREATE OR REPLACE FUNCTION private.update_grade(int, varchar(20), text, text, int)
        RETURNS TABLE (id int, name_en varchar(20), name_ru varchar(20), background text, object_key text, order_number int)
        AS $$
        BEGIN
    ...
    CREATE OR REPLACE FUNCTION private.update_subject(int, varchar(20), text, text, int)
        RETURNS TABLE (id int, fk int, name_en varchar(20), name_ru varchar(20), background text, object_key text, order_number int)
        AS $$
        BEGIN
    ...
    CREATE OR REPLACE FUNCTION private.update_lecture(int, varchar(20), text, text, text, int)
        RETURNS TABLE (id int, fk int, name_en varchar(20), name_ru varchar(20), description text, background text, object_key text, order_number int)
        AS $$
        BEGIN
...
def remove_unique_constraint_our_team() -> None:
    ...
    CREATE OR REPLACE FUNCTION about.insert_our_team(i_order int, i_name text, i_role text, i_profession text, i_description text, i_object_key text, i_photo_link text)
        RETURNS TABLE ("order" int, name text, role text, profession text, description text, object_key text, photo_link text, id int)
        AS $$
        DECLARE 
            inserted_id int;
        BEGIN 
        INSERT INTO about.our_team ("order", name, role, profession, description, object_key, photo_link)
        VALUES (i_order, i_name, i_role,i_profession, i_description, i_object_key, i_photo_link) RETURNING about.our_team.id INTO inserted_id;
    ...
    # update team member
    op.execute('''
    CREATE OR REPLACE FUNCTION about.update_team_member(id_ int, i_order int, i_name text, i_role text, i_profession text, i_object_key text, i_photo_link text,  i_description text)
        RETURNS TABLE ("order" int, name text, role text, profession text, description text, object_key text, photo_link text, id int)
        AS $$
        BEGIN
